---
title: 'Fig4 correlation plots between mouse ex clusters and high res chicken ex clusters '
output:
  html_document:
    df_print: paged
  pdf_document:
...
mouse<-subset(merged,idents=c(grep('Inh',levels(merged$final.clusters2),value = T)),invert=T)
...
Idents(chicken2)<-'merged.res.3.m.renamed'
...
#save(chickenDEGs,file='chickenDEGs_exmvsMouse.RData')

#mouseDEGs<-FindAllMarkers(mouse2,test.use = 'wilcox')
#save(mouseDEGs,file='mouseDEGs_exmvsMouseinclGly.RData')



load('data/chickenDEGs_exmvsMouse.RData')
load('data/mouseDEGs_exmvsMouseinclGly.RData')
```
...
ha_bottom<- HeatmapAnnotation(
  cl=colnames(mat),
  col=list(cl=setNames(RLpal[c(1:15)],colnames(mat))),
...
ha_right<- rowAnnotation(
  cl=rownames(mat),
  col=list(cl=setNames(RLpalchicken,rownames(mat))),
  simple_anno_size=unit(2,"mm")
)
