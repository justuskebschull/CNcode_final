function [numareas]=plotsortedheatmap(data_l,data_r,dcn,top,names,DCN,pl)
numareas=[];
for i=1:numel(DCN)
    %left hemi
    dl=data_l(:,dcn==i);
...
%         t=ttest(dl',0,'Alpha',0.05/size(data_l,1));
    ...
    colorbar;
    if pl==1
    print(gcf,['figure_exports/',DCN{i},'_contra'],'-dpdf','-r1000','-fillpage')
    end
...
%         t=ttest(dr',0,'Alpha',0.05/size(data_r,1));
    ...
    colorbar;
    if pl==1
    print(gcf,['figure_exports/',DCN{i},'_ipsi'],'-dpdf','-r1000','-fillpage')
    end
end