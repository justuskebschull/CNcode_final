...
s = imfinfo('../classAscificlo-Thalamus_projections.tif');
...
rawdatafull = (zeros(s(1).Height,s(1).Width,numel(s)));
    ...
    for i = 1:numel(s) 
        tmp=imread('../classAscificlo-Thalamus_projections.tif',i);
        rawdataL(:,:,i) = tmp(:,1:57);
...
%% remove "thalamic" areas. kind of self projections and plot bargraph.contra
...
%% remove "thalamic" areas. kind of self projections and plot bargraph.ipsi
thal=findchildren(549,annotated,pathId);
i=ismember(annotated_collapsed.id,thal);
A=AxonsByRegionL(~i);
B=RegionalDensityL(~i);
Nt=annotated_collapsed.name(~i);
